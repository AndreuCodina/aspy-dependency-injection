...
class FastApiDependencyInjection:
    ...
    async def _resolve_injected_parameter(
        ...
        parameter_service = await cls._get_request_container().service_provider.get_service_object(
            parameter_information.parameter_type
        )

        if parameter_service is None:
            if parameter_information.has_default:
                return parameter_information.default_value
            ...
            error_message = (
                ...
                f"while attempting to invoke endpoint."
            )
